<%@page import="com.manipal.service.impl.EmployeeServiceImp"%>
...
<head>
    ...
    <title>ViewCustomer | Shoptos</title>
...
<table id="customers">
	<b>EMPLOYEE TABLE INFORMATION</b><br><br>
	<tr>
		<td>EMPLOYEE_ID</td>
	...
	<% EmployeeServiceImp view=new EmployeeServiceImp();
	List<Employee> list=view.viewDatas();
	EmployeeServiceImp view1=new EmployeeServiceImp();
	List<Address> list1=view1.viewDatas1();
	for(Employee cal:list)
	{ %>
    		...
    		<tr>
		<td><%out.println(cal.getEmployee_Id());%></td>
		<td><%=cal.getEmployee_Username()%></td>
		<td><%=cal.getEmployee_Password()%></td>
		<td><%=cal.getEmployee_First_Name()%></td>
		<td><%=cal.getEmployee_Last_Name()%></td>
		<td><%=cal.getEmployee_Date_Of_Birth()%></td>
		<td><%=cal.getEmployee_Contact_No()%></td>
		<td><%=cal.getEmployee_Email()%></td>
		
		<%-- <td><%=cal.getEmployee_Address_Id()%></td> --%>
		<% 
		for(Address add:list1){
			if(cal.getEmployee_Address_Id()==add.getAddress_Id()){%>
				<td><%=add.getAddress()%></td>
				<td><%=add.getCity()%></td>
				<td><%=add.getState()%></td>
					<td><%=add.getCountry()%></td>
				<td><%= add.getZipcode()%></td>
				<td><%=add.getStatus() %></td>	
	<%}
			}
%> 
<td><%=cal.getEmployee_Status() %></td>
</tr>
<%} %>
